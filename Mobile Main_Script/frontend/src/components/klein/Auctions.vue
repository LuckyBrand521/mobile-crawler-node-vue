        ...
        <v-card>
          ...
          <v-overlay v-if="similarity" :value="similarity" dark=false>
            <v-card>
              ...
              <v-card-text><CarSimilarity :auction_id="similarity.auction_id" table_name="klein"/></v-card-text>
            </v-card>
          </v-overlay>
          <v-data-table
            :headers="headers"
  ...
  export default {
    ...
    methods: {
      ...
      async getAuctions() {
        ...
        const req = await fetch(`${process.env.VUE_APP_API}klein/auctions?${params.toString()}`);
        const data = await req.json();
        return data;
      },

      async getParameters() {
        const req = await fetch(`${process.env.VUE_APP_API}klein/parameters`);
        const data = await req.json();
        this.parameters = data;
      },

      async getModels(brand_id) {
        const req = await fetch(`${process.env.VUE_APP_API}klein/models/${brand_id}`);
        const data = await req.json();
        this.parameters = { ...this.parameters, models: data.models };
      },

      async getCatalog(batch_id) {
        const req = await fetch(`${process.env.VUE_APP_API}klein/catalogs/${batch_id}`);
        const data = await req.json();
        this.parameters = { ...this.parameters, catalog: data.catalog };
      },
      
      async favorite_shift(item, auction_id) {
        const req = await fetch(`${process.env.VUE_APP_API}klein/favorite_shift/${auction_id}`);
        const data = await req.json();
