      ...
      <v-col :cols="12">
        <v-card>
          ...
          <v-overlay v-if="similarity" :value="similarity" :dark="false">
            <v-card>
              <v-card-title>Similar cars as {{ similarity.version }}<v-spacer/><v-icon @click="similarity = null">{{ mdiClose }}</v-icon></v-card-title>
              <v-card-text><CarSimilarity :auction_id="similarity.auction_id" table_name="aldcarmarket"/></v-card-text>
            </v-card>
          </v-overlay>
          <v-data-table
            :headers="headers"
          ...
          >
            ...
            <template v-slot:item.batch_id="{ item }">
              <a :style="[new Date(item.end_date) < now ? { color: 'red' } : {}]" :href="`https://www.aldcarmarket.com/de-de/sale/${item.batch_id}/`" target="_blank">LINK</a>
            </template>
            <template v-slot:item.size="{ item }">
              <span>{{ item.size == 'Saloon'? 'Limousine' : item.size == 'Station wagon'? 'Kombi':item.size }}</span>
            </template>
            ...
            <template v-slot:item.first_registration="{ item }">
              <span><b>{{ dateFormat.format(new Date(item.first_registration)) }}</b></span>
            </template>
            ...
            <template v-slot:item.price_20="{ item }">
            ...
            <template v-slot:item.heating="{ item }">
              <span>{{ featureCheck(item.heating) }}</span>
            </template>
            ...
            <template v-slot:item.fuel="{ item }">
              <span>{{ item.fuel_name == 'PHEV/PETROL' || item.fuel_name == 'Petrol'? 'Benzin':item.fuel_name }}</span>
            </template>
          </v-data-table>
        </v-card>
      </v-col>
        ...
        <div class="slidein" v-if="filteropen">
          <v-form>
            ...
            <v-autocomplete
              ...
              v-model="filters.catalog_number"
  ...
  export default {
    ...
    methods: {
      ...
      setRowBackground(item) {
        return new Date(item.end_date) < now ? 'red-background' : 'green-background';
      },
      ...
      async getAuctions() {
        ...
        const req = await fetch(`${process.env.VUE_APP_API}aldcarmarket/auctions?${params.toString()}`);
        const data = await req.json();
        return data;
      },

      async getParameters() {
        const req = await fetch(`${process.env.VUE_APP_API}aldcarmarket/parameters`);
        const data = await req.json();
        this.parameters = data;
      },

      async getModels(brand_id) {
        const req = await fetch(`${process.env.VUE_APP_API}aldcarmarket/models/${brand_id}`);
        const data = await req.json();
        this.parameters = { ...this.parameters, models: data.models };
      },

      async getCatalog(batch_id) {
        const req = await fetch(`${process.env.VUE_APP_API}aldcarmarket/catalogs/${batch_id}`);
        const data = await req.json();
        this.parameters = { ...this.parameters, catalog: data.catalog };
      },
      async favorite_shift(item, auction_id) {
        const req = await fetch(`${process.env.VUE_APP_API}aldcarmarket/favorite_shift/${auction_id}`);
        const data = await req.json();
