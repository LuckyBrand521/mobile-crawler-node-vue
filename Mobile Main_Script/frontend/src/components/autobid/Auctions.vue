        ...
        <v-card>
          ...
          <v-overlay v-if="similarity" :value="similarity" :dark="false">
            <v-card>
              ...
              <v-card-text><CarSimilarity :auction_id="similarity.auction_id" table_name="autobid"/></v-card-text>
            </v-card>
          </v-overlay>
          <v-data-table
            :headers="headers"
          ...
          >
            ...
            <template v-slot:item.title="{ item }">
              <a :style="[new Date(item.end_date) < now ? { color: 'red' } : {}]" :href="item.url" target="_blank">LINK</a>
            </template>
            ...
            <template v-slot:item.gearbox="{ item }">
              <span>{{ item.gearbox_name == '5-Gang-Schaltgetriebe' || item.gearbox_name == '6-Gang-Schaltgetriebe' ? 'Schaltung' : item.gearbox_name }}</span>
            </template>
  ...
  export default {
    ...
    methods: {
      ...
      async getAuctions() {
        ...
        const req = await fetch(`${process.env.VUE_APP_API}autobid/auctions?${params.toString()}`);
        const data = await req.json();
        return data;
      },

      async getParameters() {
        const req = await fetch(`${process.env.VUE_APP_API}autobid/parameters`);
        const data = await req.json();
        this.parameters = data;
      },

      async getModels(brand_id) {
        const req = await fetch(`${process.env.VUE_APP_API}autobid/models/${brand_id}`);
        const data = await req.json();
        this.parameters = { ...this.parameters, models: data.models };
      },

      async getCatalog(batch_id) {
        const req = await fetch(`${process.env.VUE_APP_API}autobid/catalogs/${batch_id}`);
        const data = await req.json();
        this.parameters = { ...this.parameters, catalog: data.catalog };
      },

      async favorite_shift(item, auction_id) {
        const req = await fetch(`${process.env.VUE_APP_API}autobid/favorite_shift/${auction_id}`);
        const data = await req.json();
        if(data.msg == 'success'){
