...
async function getSimilarCars(auction_id: string) {
  ...
  FROM aldcarmarketcrawler.auction a, mobilecrawler.car c
  WHERE a.car_brand = c.car_brand AND a.car_model = c.car_model AND a.auction_id = ${auction_id}
  ORDER BY score DESC, text_match DESC
  LIMIT 20;
  `;
  await connection.query(`SET @tmp = (SELECT version FROM aldcarmarketcrawler.auction a WHERE a.auction_id = ${auction_id});`);
...
router.get('/auctions', async (req, res) => {
  ...
  const where = buildWhere("a", filters, null, null);
...
router.get('/favorite_shift/:auction_id', async(req, res) => {
  const { auction_id } = req.params;
  const sql = `UPDATE aldcarmarketcrawler.auction 
...
router.get('/parameters', async (req, res) => {
  const [batches] = await connection.query("SELECT batch_id as value, batch_id as text FROM auction_batch");
  ...
  return res.send({ batches, brands, fuels, gearboxes, prices, mileages, registration_dates, powers, seats, leathers });
});
