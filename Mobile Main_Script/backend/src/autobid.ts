...
(async () => {
  connection = await connectToDB(databases.autobid);
})();

async function getSimilarCars(auction_id: string) {
  const sql = `
  SELECT a.auction_id, IF(a.fuel = c.fuel, 1, 0) + IF(a.gearbox = c.gearbox, 1, 0) + IF(a.leder = c.leather, 1, 0) + IF(a.power = c.power, 1, 0) + IF(a.four_wheel = c.four_wheel, 1, 0) + IF(a.power = c.power, 1, 0) + IF(c.kilometer BETWEEN a.mileage-25000 AND a.mileage+25000, 1, 0) + IF(c.reg_date = YEAR(a.first_registration), 1, 0) + IF(a.navigation = c.navigation, 1, 0) as score, MATCH(c.car_title) AGAINST (@tmp IN BOOLEAN MODE) as text_match, c.*
  FROM autobidcrawler.auction a, mobilecrawler.car c
  WHERE a.car_brand = c.car_brand AND a.car_model = c.car_model AND a.auction_id = ${auction_id}
  ORDER BY score DESC, text_match DESC
  LIMIT 20;
  `;
  await connection.query(`SET @tmp = (SELECT title FROM autobidcrawler.auction a WHERE a.auction_id = ${auction_id});`);
...
router.get('/auctions', async (req, res) => {
  ...
  const sql = `SELECT a.*, m.model_name as model_name, b.brand_name as brand_name, g.name as gearbox_name, f.name as fuel_name, l.name as leather_name
    FROM auction a
    ...
    LEFT JOIN mobilecrawler.leathers l on (a.leder
 = l.id) 
    ${where} 
    ${order} 
    ${limit}`;
...
router.get('/auctions/:auction_id/history', async (req, res) => {
  ...
  const sql = `SELECT a.*, g.name as gearbox_name, f.name as fuel_name, l.name as leather_name
...
router.get('/favorite_shift/:auction_id', async(req, res) => {
  const { auction_id } = req.params;
  const sql = `UPDATE autobidcrawler.auction 
...
router.get('/parameters', async (req, res) => {
  const [batches] = await connection.query("SELECT DISTINCT batch_id as value, batch_id as text FROM auction");
  ...
  const [catalogs] = await connection.query("SELECT DISTINCT catalog_number as value, catalog_number as text FROM auction ORDER BY `catalog_number` ASC");
  return res.send({ batches, brands, fuels, gearboxes, prices, mileages, registration_dates, powers, seats, leathers, catalogs });
});
...
router.get('/catalogs/:batch_id', async (req, res) => {
  ...
  const [catalog] = await connection.query(`SELECT catalog_number as value, catalog_number as text FROM auction WHERE batch_id = '${batch_id}' ORDER BY catalog_number`);
