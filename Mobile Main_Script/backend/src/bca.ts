...
async function getSimilarCars(auction_id: string) {
  const sql = `
  SELECT a.auction_id, IF(a.fuel = c.fuel, 1, 0) + IF(a.leder = c.leather, 1, 0) + IF(a.power = c.power, 1, 0) + IF(a.power = c.power, 1, 0) + IF(c.kilometer BETWEEN a.mileage-25000 AND a.mileage+25000, 1, 0) + IF(c.reg_date = YEAR(a.first_registration), 1, 0) + IF(a.navigation = c.navigation, 1, 0) as score, MATCH(c.car_title) AGAINST (@tmp IN BOOLEAN MODE) as text_match, c.*
  FROM bca.auction a, mobilecrawler.car c
  WHERE a.car_brand = c.car_brand AND a.car_model = c.car_model AND a.auction_id = '${auction_id}'
  ORDER BY score DESC, text_match DESC
  LIMIT 20;
  `;
  await connection.query(`SET @tmp = (SELECT version FROM bca.auction a WHERE a.auction_id = '${auction_id}');`);
...
router.get('/auctions/:auction_id/history', async (req, res) => {
  ...
  const sql = `SELECT a.*, f.name as fuel_name,g.name as gearbox_name, l.name as leather_name
    FROM auction_history a 
    LEFT JOIN mobilecrawler.fuel f ON (a.fuel = f.id)
    LEFT JOIN mobilecrawler.gearbox g ON (a.fuel = g.id)
...
router.get('/parameters', async (req, res) => {
  const [batches] = await connection.query("SELECT batch_id as value, `name` as text FROM auction_batch");
