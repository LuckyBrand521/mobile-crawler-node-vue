...
(async () => {
  connection = await connectToDB(databases.klein);
})();
...
async function getSimilarCars(auction_id: string) {
  ...
  FROM kleinanzeigencrawler.auction a, mobilecrawler.car c
  WHERE a.car_brand = c.car_brand AND a.car_model = c.car_model AND a.auction_id = ${auction_id}
  ORDER BY score DESC, text_match DESC
  LIMIT 20;
  `;
  await connection.query(`SET @tmp = (SELECT title FROM kleinanzeigencrawler.auction a WHERE a.auction_id = ${auction_id});`);
...
router.get('/favorite_shift/:auction_id', async(req, res) => {
  const { auction_id } = req.params;
  const sql = `UPDATE kleinanzeigencrawler.auction 
