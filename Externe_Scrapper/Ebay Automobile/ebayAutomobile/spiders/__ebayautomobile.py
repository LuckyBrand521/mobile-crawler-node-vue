...
def process_product_db(item):
    try:
        ...
        model_existing_query = f"SELECT model_id, model_name FROM mobilecrawler.model WHERE model_name='{item['car_model']}' and brand_id={brand_id}"
        ...
        if model_result:
            ...
            if item["car_model"] == '':
                ...
                max_model_id_query = f"SELECT MAX(model_id) FROM mobilecrawler.model"
                mycursor.execute(max_model_id_query)
                max_id_result = mycursor.fetchone()
                max_model_id = max_id_result[0] + 1
                # insert new brand name and id
                mycursor.execute("INSERT INTO mobilecrawler.model (model_id, model_name, brand_id) VALUES (%s,%s,%s)", (max_model_id, item['car_model'], brand_id))
                mydb.commit()
            ...
            for attr_tr in attr_table.xpath(".//tr"):
                for index, attr_td in enumerate(attr_tr.xpath("./td")):
                    ...
                    if attr_td.xpath("@class").get() == "attrLabels":
                        ...
                        elif "Antriebsart" in td_text or "Besonderheiten" in td_text:
