    ...
    def fake_response(self, response):
        ...
        for input_row in input_data:
            ...
            if input_row["first_registration"]:
                input_year = input_row["first_registration"]
            ...
            if input_row["power"]: 
                min_kw_value = int((int(input_row['power']) - 3) / 1.36) # I search on the internet, 1KW=1.36PS
                max_kw_value = int((int(input_row['power']) + 3) / 1.36)
                # the order affect the result.
                param_list.append(f"minPowerAsArray={int(input_row['power']) - 3}")
                param_list.append("minPowerAsArray=PS")
                # Set the high bound of the range to low + 9.
                param_list.append(f"maxPowerAsArray={int(input_row['power']) + 3}")
